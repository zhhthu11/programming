...
class TreeNode{
  ...
  public:
    ...
    void ModifyTreeNode(int modify_index, int modify_value){
        if (range_left_ == range_right_){
            node_score_.left_joint_score = modify_value;
            node_score_.right_joint_score = modify_value;
            node_score_.total_score = modify_value;
            node_score_.best_score = modify_value;
            return;
        }

        int range_middle = (range_left_ + range_right_) / 2;
        if (modify_index <= range_middle)
            left_node_->ModifyTreeNode(modify_index, modify_value);
        if (modify_index >= range_middle + 1)
            right_node_->ModifyTreeNode(modify_index, modify_value);
        tree_node_score left_node_score = left_node_->GetNodeScore();
        tree_node_score right_node_score = right_node_->GetNodeScore();
